...
- ✅ カスタムドメイン設定（ACM + CloudFront、vibe.er.ht）
...
| ページ | URL |
| --- | --- |
| **メインサイト (カスタムドメイン)** | [https://vibe.er.ht](https://vibe.er.ht) |
| **メインサイト (CloudFront)** | [https://d2opqv3ja0x6v5.cloudfront.net](https://d2opqv3ja0x6v5.cloudfront.net) |
| **404エラーページ** | [https://vibe.er.ht/404.html](https://vibe.er.ht/404.html) |
...
│   404 page      │  │   Images        │  │   User Pool     │  │  RDS認証情報    │
│   + OAC         │  │   + OAC         │  │   50K MAU無料   │  │                 │
└─────────────────┘  └─────────────────┘  └─────────────────┘  └─────────────────┘
```
...
| Cognito | 50,000 MAU無料（永久） |
| S3 | 5GBストレージ、20,000 GET |
| CloudFront | 1TB転送、10,000,000リクエスト |
| ECR | 500MBストレージ |
| Secrets Manager | 30日間無料トライアル |

### MVP vs Production構成比較

| 項目 | MVP（現在） | Production推奨 |
| --- | --- | --- |
| **RDS配置** | Public Subnet (単一AZ) | Private Subnet (Multi-AZ) |
| **RDS Failover** | なし | 自動Failover (1-2分) |
| **Read Replica** | なし | 読み取り分散用に推奨 |
| **NAT Gateway** | なし（コスト削減） | 必須（~$30-45/月） |
| **サブネット構成** | 10.0.1.0/24, 10.0.2.0/24 | +10.0.11.0/24, 10.0.21.0/24 (DB用) |
| **WAF** | なし | AWSManagedRulesCommonRuleSet |
| **バックアップ** | 7日間 | 30日間 + クロスリージョン |

> ⚠️ **注意**: 本番環境ではPrivate Subnet + NAT Gateway構成を強く推奨します。
> 現在のPublic Subnet配置はコスト最適化のための選択であり、Security Groupで厳密にアクセス制御しています。
